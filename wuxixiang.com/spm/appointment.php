...
<?php include_once('navbar.php'); ?>
    ...
    <body>
        ...
        <head>
            <meta charset="utf-8">
            <title>Dog-Clinic Appointment</title>
        ...
        <p>Choose time in the calendar</p>
        <script>
        </script>
        <!--Panel starts here-->
            ...
            <div class="col-sm-6 col-sm-offset-3">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title">Preffered Date and time</h3> </div>
                    <div id="datePicker" class="panel-body">Time you preferred</div>
                </div>
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title">Preffered Date and time</h3> </div>
                    <div id="timePicker" class="panel-body"></div>
                </div>
                <div class="panel panel-default">
                    <div class="panel-heading">
                ...
                <select id="groom" class="selectpicker">
                    <option>Washing</option>
                    <option>Deluxe Washing</option>
                    <option>Nail Clipping</option>
                </select>
                <select id="dogLst" class="selectpicker"> </select>
                <input id="notes" type="text" rows="5">
                <button onClick="new_appointment()" type="button">Create appointment</button>
            </div>
        </form>
        <!--Panel finishes here-->
    </body>
...
//        retrieve_user_profile();
        ...
        this.label = [];
        this.labels = [];
        for (var i = 0; i < 7; i++) this.label.push(label[(label.indexOf(labelSettings[0]) + this.label.length >= label.length) ? Math.abs(label.length - (label.indexOf(labelSettings[0]) + this.label.length)) : (label.indexOf(labelSettings[0]) + this.label.length)]);
        for (var i = 0; i < 7; i++) this.labels.push(this.label[i].substring(0, (labelSettings[1] > 3) ? 3 : labelSettings[1]));
        this.date = new Date();
        this.draw();
        this.update();
    }
    ...
    Calendar.prototype.update = function () {
        ...
        //        var datePrint = this.date.getDay + '/' + this.date.getMonth + '/' + this.date.getFullYear;
        document.getElementById("datePicker").innerHTML = this.date.toString().split(" ").slice(0, 4).join(" ");;
        ...
        for (i = 1; i <= lastDay; i++) {
            ...
            if (i == this.date.getDate()) document.getElementById(this.id + '-day-radio-' + (this.label.indexOf(label[firstDay]) + i)).checked = true;
        }
        for (i = lastDay + 1, j = 1;
    ...
    Organizer.prototype.update = function () {
        ...
        // change the date picker to today
        //        changeDatePanel(0,0);
    }
    Organizer.prototype.list = function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].text == "Free") {
                content += '<li id="' + this.id + '-list-item-' + i + '"><a style="background-color:rgb(204, 255, 204);padding-left: 5px;padding-top: 5px;padding-right: 5px;padding-bottom: 0px;width: 100%;" onclick="chose_a_time(\'' + data[i].startTime + '\')" value="' + data[i].startTime + '"><div><span class="' + this.id + ' time" id="' + this.id + '-list-item-' + i + '-time">' + data[i].startTime + ' - ' + data[i].endTime + '</span><span class="' + this.id + ' m" id="' + this.id + '-list-item-' + i + '-m">' + data[i].mTime + '</span></div><p id="' + this.id + '-list-item-' + i + '-text">' + data[i].text + '</p></a></li>';
            }
            else {
                content += '<li id="' + this.id + '-list-item-' + i + '"><a style="padding-left: 5px;padding-top: 5px;padding-right: 5px;padding-bottom: 0px;width: 100%;" onclick="chose_a_time(\'occupied\')" value="' + data[i].startTime + '"><div><span class="' + this.id + ' time" id="' + this.id + '-list-item-' + i + '-time">' + data[i].startTime + ' - ' + data[i].endTime + '</span><span class="' + this.id + ' m" id="' + this.id + '-list-item-' + i + '-m">' + data[i].mTime + '</span></div><p id="' + this.id + '-list-item-' + i + '-text">' + data[i].text + '</p></a></li>';
            }
        }
    ...
    function generateData() {
        ...
        var c_date = document.getElementById("datePicker").innerHTML;
        console.log(c_date);
        if (c_date == null) {c_date = new Date();}
        
        var monthLst = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
        
        
        var num_year, num_mon, num_day;
            
            num_year = c_date.split(" ")[3];
            num_mon  = monthLst.indexOf(c_date.split(" ")[1]) + 1;
            num_day  = c_date.split(" ")[2];
        ...
        for (var i = 0; i < 6; i++) {
            ...
            var c_hour = stSum/minute;
            var c_min  = stSum%minute;
            
            if (c_min==0) {
                tmp_event["startTime"] = c_hour.toString()+':'+'00';    
            } else {
                tmp_event["startTime"] = (c_hour-0.5).toString()+':'+'30';    
            }
            ...
            if (c_min==0) {
                tmp_event["endTime"] = c_hour.toString()+':'+'00';    
            } else {
                tmp_event["endTime"] = (c_hour-0.5).toString()+':'+'30';    
            }
            
//            console.log(tmp_event["startTime"] + " " + tmp_event["endTime"]);
            ...
            var checkForm = num_year+num_mon+num_day+tmp_event["startTime"];
            
            tmp_event["text"] = checkOccupied(checkForm.split(new RegExp('[-+()*/:?0 ]', 'g')).join(''));
            
            eventsLst.push(tmp_event);
            
//            console.log(tmp_event["startTime"] + " " + tmp_event["endTime"] + " " +tmp_event["mTime"] + " " +tmp_event["text"] );

        }
        
        var daysLst = [];
        data = {years: [{int: (num_year), months: [{int: (num_mon), days: [{int: (num_day), events: eventsLst}]}]}]};
    }
    
    function checkOccupied(txt) {        
//        if (profile_dic.appTimeLst==null) {console.log("not ready!");}
        for (i in profile_dic.appTimeLst) {
            var againstText = profile_dic.appTimeLst[i].split(new RegExp('[-+()*/:?0 ]', 'g')).join('');
            if (againstText == txt ){
                return "Occupied";
            }
        }
...
//    window.onload = retrieve_user_profile();
    ...
    function chose_a_time(time) {
        document.getElementById("timePicker").innerHTML = time;
        console.log(time);
    }
    function retrieve_user_profile() {
        $.ajax({
            ...
            , success: function (res) {
                ...
                document.getElementById("id").innerHTML = profile_dic.id;
                generate_dogLst(profile_dic.dog_id_rel_lst, profile_dic.no_dog);
                generateData();
                showEvents();
            }
        });
    ...
    function generate_dogLst(dogdic, n) {
        for (var i = 0; i < n; i++) {
            var opt = document.createElement("option");
            opt.value = Object.keys(dogdic)[i];
            opt.innerHTML = Object.values(dogdic)[i];
            document.getElementById("dogLst").appendChild(opt);
        }
    }
    ...
    function new_appointment() {
        $.ajax({
            ...
            , data: {
                op: "MODI"
                , date: document.getElementById("datePicker").innerHTML
                , time: document.getElementById("timePicker").innerHTML
                , id: document.getElementById("id").innerHTML
                , dog: document.getElementById("dogLst").value
